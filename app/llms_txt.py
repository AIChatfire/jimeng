...
def render(settings: Settings) -> str:
    ...
    add("")
    add("```bash")
    add("TASK=$(curl -sS -X POST http://<host>/async/v1/videos/generations \\")
    add("  -H \"Authorization: Bearer $JIMENG_API_KEY\" -H 'Content-Type: application/json' \\")
    add("  -d '{\"model\":\"jimeng:t2v\",\"prompt\":\"海浪拍岸\"}' | jq -r .id)")
    add("curl -sS http://<host>/async/v1/videos/generations/$TASK -H \"Authorization: Bearer $JIMENG_API_KEY\"")
    add("```")
    add("")
    add("**③ 字节风格兼容面**（`/api/v3/contents/generations/tasks`，形态同方舟）")
    add("")
    add("| 形态 | 端点 |")
    add("|---|---|")
    add("| 图片（同步） | `POST /v1/images/generations` |")
    add("| 图片（异步） | `POST /async/v1/images/generations` → `GET /async/v1/images/generations/{task_id}` |")
    add("| 视频（异步） | `POST /async/v1/videos/generations` → `GET /async/v1/videos/generations/{task_id}` |")
    add("| 兼容（受理） | `POST /api/v3/contents/generations/tasks` |")
    add("| 兼容（查询） | `GET /api/v3/contents/generations/tasks/{task_id}` |")
    add("")
    add("## 鉴权")
    add("")
